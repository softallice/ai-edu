...
> 본 하네스에서 어떤 기능 개발/리팩터링/버그수정에도 적용할 수 있는 단계·게이트·반복 워크플로우를 정의합니다.
...
4. **상태 영속화**: 매 단계 종료 시 `.pdca-status.json`을 갱신한다(`.pdca-status.json` 파일을 Read/Write 로 직접 갱신).
...
- **종료 시**: `.pdca-status.json` 파일에 `currentPhase=plan, done` 기록.
...
- **전제**: design 문서 존재.
- **할 일**: 설계대로 코드 작성. 작은 파일·불변성·입력검증·에러 처리(CLAUDE.md·컨벤션 팩 준수). 가능하면 테스트도 함께.
...
- **MANDATORY**: `.pdca-status.json` 파일에 `completed=true`, `completedAt`, `generatedFiles`, gate 점수 최종 기록.
...
- **Q. 컨텍스트가 부족해지면?** `strategic-compact` 스킬과 단계별 핸드오프 문서를 활용해 단계 간 컨텍스트를 압축 전달합니다.

## 관련 파일

- `config/pdca.config.json` — 단계·게이트·임계값 설정
- `.claude/agents/pdca-orchestrator.md` — 오케스트레이터 에이전트
- `.claude/commands/pdca*.md` — 커맨드 템플릿
- `CLAUDE.md` + 컨벤션 팩 — 코딩/보안/테스트 규칙(품질 게이트 근거)
