...
/** 업적평가 동적 검색 조건. */
public final class AppraisalSpecifications {
    private AppraisalSpecifications() {}

    /** code 키워드 like 검색. */
    public static Specification<Appraisal> keyword(String kw) {
        return (root, q, cb) -> {
    ...
    /** 상태 일치 검색. */
    public static Specification<Appraisal> statusEquals(AppraisalStatus status) {
        return (root, q, cb) -> status == null ? cb.conjunction()
                : cb.equal(root.get("status"), status);
    }

    /** 직원 ID 일치 검색. */
    public static Specification<Appraisal> employeeEquals(Long employeeId) {
        return (root, q, cb) -> employeeId == null ? cb.conjunction()
                : cb.equal(root.get("employee").get("id"), employeeId);
    }

    /** 평가기간 일치 검색(String 완전 일치). */
    public static Specification<Appraisal> periodEquals(String period) {
