...
const STATE_ITEMS = (
  Object.entries(TAX_INVOICE_STATUS) as [TaxInvoiceStatus, string][]
).map(([value, label]) => ({ label, value }))

export function TaxInvoiceIssuePage() {
  ...
  const save = useSaveTaxInvoice()
  const remove = useDeleteTaxInvoice()

  const [open, setOpen] = useState(false)
  const [editId, setEditId] = useState<number | undefined>(undefined)
  const [form, setForm] = useState<TaxInvoiceInput>(EMPTY)
  ...
  const set = (k: keyof TaxInvoiceInput, v: unknown) =>
    setForm((f) => ({ ...f, [k]: v }))
  ...
  const submit = () => {
    if (!form.customerId || !(form.supplyAmount > 0)) {
      toast.error('거래처와 공급가액(0 초과)은 필수입니다.')
      return
    }
    save.mutate(
      { id: editId, body: { ...form, issueDate: form.issueDate || null } },
      {
        onSuccess: () => {
          toast.success(
            editId ? '세금계산서를 수정했습니다.' : '세금계산서를 발행했습니다.'
          )
          setOpen(false)
        },
  ...
  const onDelete = (t: TaxInvoice) => {
    if (!confirm(`세금계산서 [${t.code}]을(를) 삭제하시겠습니까?`)) return
    remove.mutate(t.id, {
      ...
      <Main className='flex flex-1 flex-col gap-4'>
        <div className='flex flex-wrap items-end justify-between gap-2'>
          <div>
            ...
            <p className='text-muted-foreground'>02.영업 / 세금계산서 — 발행</p>
          </div>
