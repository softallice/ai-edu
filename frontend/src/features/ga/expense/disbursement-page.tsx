...
const EMPTY: ExpenseRequestInput = {
  employeeId: 0,
...
const TYPE_ITEMS = (
  Object.entries(EXPENSE_TYPE) as [ExpenseType, string][]
...
export function ExpenseDisbursementPage() {
  ...
  const [form, setForm] = useState<ExpenseRequestInput>(EMPTY)
  ...
  const submit = () => {
    ...
    save.mutate(
      { id: editId, body: { ...form, requestDate: form.requestDate || null } },
      {
        onSuccess: () => {
          toast.success(
            editId ? '지출품의를 수정했습니다.' : '지출품의를 등록했습니다.'
          )
          setOpen(false)
        },
  ...
  const onDelete = (r: ExpenseRequest) => {
    if (!confirm(`지출품의 [${r.code}]을(를) 삭제하시겠습니까?`)) return
    remove.mutate(r.id, {
    ...
    <>
      ...
      <Main className='flex flex-1 flex-col gap-4'>
        <div className='flex flex-wrap items-end justify-between gap-2'>
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>지출품의</h2>
            <p className='text-muted-foreground'>07.총무 / 지출품의</p>
          </div>
          <div className='flex items-end gap-2'>
            <Input
              placeholder='품의번호·제목 검색'
              value={keyword}
              onChange={(e) => setKeyword(e.target.value)}
              className='w-48'
            />
            <Button onClick={openCreate}>
              <Plus size={16} /> 지출품의 등록
            </Button>
          </div>
        </div>
        ...
        <div className='overflow-hidden rounded-md border'>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className='w-36'>품의번호</TableHead>
                <TableHead className='w-32'>신청자</TableHead>
                <TableHead className='w-24'>비용유형</TableHead>
