...
import formatCodeTool from "../tools/format-code.js"
import lintCheckTool from "../tools/lint-check.js"
import gitSummaryTool from "../tools/git-summary.js"
import pdcaStatusTool from "../tools/pdca-status.js"
  ...
  return {
    ...
    "experimental.session.compacting": async () => {
      const contextBlock = [
        ...
        "- PDCA: plan -> design -> do -> check -> iterate -> report; L3 (tests/behavior) is the hard gate",
        "",
      ]

      // Preserve active PDCA progress (critical for the limited-context local model)
      try {
        const statusFile = resolvePath(".pdca-status.json")
        if (fs.existsSync(statusFile)) {
          const store = JSON.parse(fs.readFileSync(statusFile, "utf-8")) as {
            features?: Record<string, { currentPhase?: string; completed?: boolean; iterationCount?: number; gates?: Record<string, number>; notes?: string }>
          }
          const entries = Object.entries(store.features || {})
          if (entries.length > 0) {
            contextBlock.push("## Active PDCA Progress (.pdca-status.json)")
            for (const [id, s] of entries) {
              const gates = s.gates ? Object.entries(s.gates).map(([k, v]) => `${k}=${v}`).join(" ") : "-"
              contextBlock.push(`- ${id}: phase=${s.currentPhase ?? "?"} completed=${!!s.completed} iter=${s.iterationCount ?? 0} gates[${gates}]${s.notes ? ` notes="${s.notes}"` : ""}`)
            }
            contextBlock.push("")
          }
        }
      } catch {
        // Status file unreadable — skip silently
      }
      ...
      return {
        ...
        compaction_prompt: "Focus on preserving: 1) Active PDCA phase, quality-gate scores, and iteration count per feature, 2) Current task status and progress, 3) Key decisions made, 4) Files created/modified, 5) Remaining work items and any failing gate (especially L3 tests/behavior), 6) Any security concerns flagged. Discard: verbose tool outputs, intermediate exploration, redundant file listings.",
      }
    },
