...
### 최종 — 그리드 데이터 + 색상 표시 ✅
- 그리드 포맷을 `_setContents("<Formats>...")`로 **생성 시점**에 설정(스크립트 set_format 대신) → 데이터 행 정상 렌더.
- 메인 표시 확인(스크린샷): 좌측 메뉴(구매/거래처등록), 거래처 목록 3건(코드/거래처명/구분/대표자), 담당자 그리드 헤더, 다크 상단바 + 파란 버튼 + 테마 줄무늬.
- 색상/스타일은 `set_background`/`set_color` 및 inbus 테마로 적용. 헤드리스 스크린샷의 □□는 폰트 부재 아티팩트(실제 브라우저는 정상).

### 데모 — 전체 프레임/로그인/메인/프로그램 마이그레이션 ✅

레거시 Nexacro ERP 전체 구조를 모던 백엔드 연동으로 이관 가능함을 보이는 데모:

- **전체 프레임**: 상단바(로고/사용자/로그아웃) + 좌측 카테고리 메뉴 + 작업영역(MDI 스타일)
- **로그인**: comLogin → `ComLogin_Login.do`(JWT)
- **메인화면(홈 대시보드)**: comMain → `ComMain_Stats.do` — 거래처 수/사용중/이관 프로그램 통계 카드
- **메뉴**: `ComLogin_Menu.do` — 카테고리형(구매관리·기준정보·시스템관리) 10개 항목, 클릭 시 작업영역에 로드(미이관 항목은 "준비중" 안내)
- **프로그램**: 거래처등록(POVM0001) — 실제 이관·동작(목록 3건 + 담당자 그리드)

스크린샷 검증: 로그인→홈 대시보드(카드)→메뉴 클릭→거래처등록(데이터) 전 과정 동작, 모든 트랜잭션 200.
